  ...
  const handleOpenModal = (tag?: Tag) => {
    if (tag) {
      ...
      setFormData({
        name: tag.name,
        slug: tag.slug,
        nameHindi: tag.nameHindi || '',
        namePunjabi: tag.namePunjabi || '',
          ...
          <tbody className="divide-y divide-dark-800">
            {tags.map((tag) => (
              <tr key={tag.id} className="hover:bg-dark-800/50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-dark-300">
                  <div>{tag.nameHindi || '-'}</div>
                  <div className="text-dark-500">{tag.namePunjabi || '-'}</div>
                </td>
          ...
          <div className="bg-dark-900 rounded-t-2xl sm:rounded-xl w-full sm:max-w-md max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  type="text"
                  value={formData.namePunjabi}
                  onChange={(e) => setFormData({ ...formData, namePunjabi: e.target.value })}
                  className="input"
                  placeholder="Punjabi translation (optional)"
                />
              </div>
