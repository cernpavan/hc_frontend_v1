...
const tagLabels: Record<string, { en: string; hi: string; pa: string }> = {
  'sexual-confession': { en: 'Sexual Confession', hi: 'यौन कन्फेशन', pa: 'ਜਿਨਸੀ ਇਕਰਾਰ' },
  'fantasy-kinks': { en: 'Fantasy / Kinks', hi: 'फैंटसी / किंक्स', pa: 'ਫੈਂਟਸੀ' },
  'relationship-affair': { en: 'Relationship', hi: 'रिश्ता / अफेयर', pa: 'ਰਿਸ਼ਤਾ' },
  'guilt-regret': { en: 'Guilt / Regret', hi: 'अपराध बोध / पछतावा', pa: 'ਦੋਸ਼ / ਪਛਤਾਵਾ' },
  'cheating': { en: 'Cheating', hi: 'धोखा', pa: 'ਧੋਖਾ' },
  'one-night-story': { en: 'One Night', hi: 'एक रात की कहानी', pa: 'ਇੱਕ ਰਾਤ' },
  'adult-advice': { en: 'Adult Advice', hi: 'वयस्क सलाह', pa: 'ਬਾਲਗ ਸਲਾਹ' },
  'dark-thoughts': { en: 'Dark Thoughts', hi: 'गहरे विचार', pa: 'ਹਨੇਰੇ ਵਿਚਾਰ' },
  'curiosity-question': { en: 'Curiosity', hi: 'जिज्ञासा', pa: 'ਉਤਸੁਕਤਾ' },
};

// Helper to format unknown tags
const formatTagLabel = (tag: string, lang: 'hindi' | 'english' | 'punjabi'): string => {
  const knownTag = tagLabels[tag];
  if (knownTag) {
    return lang === 'hindi' ? knownTag.hi : lang === 'punjabi' ? knownTag.pa : knownTag.en;
  }
...
function PostCardComponent({ post, showFullContent = false }: PostCardProps) {
  ...
  const currentLanguage = (isAuthenticated ? user?.language : (typeof window !== 'undefined' ? localStorage.getItem('guest-language') : null) || 'hindi') as 'hindi' | 'english' | 'punjabi';
  ...
  const content = showTranslation
    ? (currentLanguage === 'hindi' ? post.contentTranslated?.hindi : currentLanguage === 'punjabi' ? post.contentTranslated?.punjabi : post.contentTranslated?.english) || post.content
    : post.content;
    ...
    >
      ...
      <div className="post-tags flex flex-wrap gap-1 md:gap-2 mb-2 md:mb-3">
        {post.tags.map((tag) => (
          ...
          >
            {formatTagLabel(tag, currentLanguage)}
          </Link>
        ))}
      ...
      {/* Images - Optimized for LCP and CLS */}
        ...
        >
          {post.images.map((img, idx) => (
            <img
              ...
              src={img}
              alt=""
              width={post.images?.length === 1 ? 640 : 320}
              height={post.images?.length === 1 ? 360 : 180}
